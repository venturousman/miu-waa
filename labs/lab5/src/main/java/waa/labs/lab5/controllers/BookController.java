...
import waa.labs.lab5.services.IBookService;
...
//        return bookService.getBooks().stream().sorted(Comparator.comparingDouble(Book::getPrice)).toList();
        var books = bookService.getBooksStream();
        return books.sorted(Comparator.comparingDouble(Book::getPrice)).toList();
    }
...
//        return bookService.getBooks().stream().sorted(Comparator.comparing(Book::getTitle)).toList();
        var books = bookService.getBooksStream();
        return books.sorted(Comparator.comparing(Book::getTitle)).toList();
    }
}